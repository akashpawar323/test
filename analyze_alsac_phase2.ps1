...
function Get-GroupColumn {
    ...
    # Use exact known schema names only. Never fall back to metric columns such
    # as pr.reviewed, because that can silently produce nonsense groupings.
    if ($FileName -like "*ai_coding*") {
        foreach ($c in @("Tool name","tool_name","coding_assistant")) {
            if ($Columns -contains $c) { return $c }
        }
        return $null
    }

    if ($FileName -like "*ai_code_review*") {
        foreach ($c in @("Tool name","tool_name","ai_review")) {
            if ($Columns -contains $c) { return $c }
        }
        return $null
    }

    if ($FileName -like "*agentic*") {
        foreach ($c in @("Tool name","tool_name","agentic_pr")) {
            if ($Columns -contains $c) { return $c }
        }
        return $null
    }

    if ($FileName -like "*developer*") {
        foreach ($c in @("internal_user_id","unified_user_id","unified_user","user_id")) {
            if ($Columns -contains $c) { return $c }
        }
        return $null
    }

    if ($FileName -like "*team*") {
        foreach ($c in @("team_name","team_id","team")) {
            if ($Columns -contains $c) { return $c }
        }
        return $null
    }

    if ($FileName -like "*repository*") {
        foreach ($c in @("repository_name","repo_name","repository_id","repository")) {
            if ($Columns -contains $c) { return $c }
        }
        return $null
    }

    if ($Columns -contains "organization_id") { return "organization_id" }
    return $null
}
...
if ($FileData.Contains("06_alsac_developer_metrics.csv")) {
    ...
    if ($Analysis.Contains("overall_baseline")) {
        $orgMerged = $Analysis["overall_baseline"].metrics["pr.merged"].full_interval_total
        $userMerged = $Analysis["developer_metrics"].pr_merged_total
        $coverage = Safe-Divide $userMerged $orgMerged
        $Analysis["developer_metrics"]["merged_pr_coverage_vs_org_pct"] =
            if ($null -ne $coverage) { $coverage * 100 } else { $null }
    }
}
...
if ($Analysis.Contains("developer_metrics")) {
    ...
    if ($null -ne $dev.merged_pr_coverage_vs_org_pct) {
        $Md.Add("- Developer-export merged-PR coverage vs organization baseline: **$(Format-Percent $dev.merged_pr_coverage_vs_org_pct)**")
        if ([double]$dev.merged_pr_coverage_vs_org_pct -lt 90) {
            $Md.Add("- **WARNING:** developer export is partial relative to organization baseline; do not treat the tier as ALSAC-wide.")
        }
    }
    $Md.Add("")
}
...
if ($Analysis.Contains("developer_metrics")) {
    ...
    if ($null -ne $dev.merged_pr_coverage_vs_org_pct) {
        $Ss.Add(("{0,-28}: {1}" -f "User export vs org coverage", (Format-Percent $dev.merged_pr_coverage_vs_org_pct)))
        if ([double]$dev.merged_pr_coverage_vs_org_pct -lt 90) {
            $Ss.Add("WARNING: developer export does not cover the full org baseline; do NOT use this tier as org-wide ALSAC tier yet.")
        }
    }
    $Ss.Add("")
}
